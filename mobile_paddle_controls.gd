...
func _process(_delta: float) -> void:
	...
	if active_finger_index == -1:
		...
		_set_knob_ratio(
			paddle.get_horizontal_position_ratio(),
			false
		)
	...
	if event is InputEventScreenTouch:
		...
		if touch.pressed:
			...
			if active_finger_index == -1 and control_hit:
				...
				and aim_finger_index == -1
				and _begin_mobile_aim(
					touch.index,
					touch.position
				)
			):
			...
			# Release anında aim'i tekrar hesaplama.
			# Son geçerli touch/drag yönünü koruyarak launch et.
...
func _layout_control() -> void:
	...
	rail.position = Vector2(
		safe_rect.position.x
			+ (safe_rect.size.x - rail_width) * 0.5,
		safe_bottom
			- rail_height
			- mobile_extra_bottom_padding
	)
	...
	paddle.set_mobile_control_y(
		knob_center_y
			- paddle_above_knob_offset
			+ mobile_paddle_y_offset
	)
...
func _log_safe_area_once() -> void:
	...
	var safe_rect := GameManager.get_layout_safe_rect(
...
func _update_from_touch_x(touch_x: float) -> void:
	var travel_width: float = maxf(
		1.0,
		rail.size.x - knob_diameter
	)

	var local_center_x: float = (
		touch_x
		- rail.global_position.x
	)
...
func _set_knob_ratio(
	...
	if drive_paddle:
		paddle.set_mobile_slider_ratio(
			knob_ratio
		)
...
func _update_knob_position() -> void:
	...
	var paddle_limits: Vector2 = (
		paddle.get_horizontal_limits()
	)

	var mapped_center_x := lerpf(
		paddle_limits.x,
		paddle_limits.y,
		knob_ratio
	)
...
func _begin_mobile_aim(
	finger_index: int,
	touch_position: Vector2
) -> bool:
	if (
		get_tree().paused
		or not _is_gameplay_touch(touch_position)
	):
		return false

	var waiting_balls := (
		get_tree().get_nodes_in_group(
			"manual_launch_waiting"
		)
	)
...
func _update_mobile_aim(
	touch_position: Vector2
) -> void:
	if (
		is_instance_valid(aimed_ball)
		and aimed_ball.has_method(
			"set_mobile_aim_target"
		)
	):
		# ScreenTouch / ScreenDrag viewport koordinatını
		# Ball'ın world canvas'ına çevir.
		var world_position := (
			aimed_ball
		...
		aimed_ball.set_mobile_aim_target(
			world_position
		)


func _cancel_mobile_aim(
	launch_on_release: bool
) -> void:
	if (
		launch_on_release
		and is_instance_valid(aimed_ball)
		and aimed_ball.has_method(
			"launch_ball"
		)
	):
...
func _is_gameplay_touch(
	touch_position: Vector2
) -> bool:
	var safe_rect := (
		GameManager.get_layout_safe_rect(
			get_viewport()
			.get_visible_rect()
			.size
		)
...
func _configure_visuals() -> void:
	...
	knob_style.set_corner_radius_all(
		int(knob_diameter * 0.5)
	)

	knob_style.shadow_color = Color(
		0.0,
		0.78,
		1.0,
		0.30
...
func _set_knob_pressed(
	...
	knob_feedback_tween.set_trans(
		Tween.TRANS_QUAD
	).set_ease(
		Tween.EASE_OUT
	)

	knob_feedback_tween.tween_property(
		knob,
		"scale",
		target_scale,
		duration
